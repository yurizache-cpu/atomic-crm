...
**Planned on `feature/phase-2c-operator-surface`, from `feature/clinical-phase-1` at `d1819f25`; planning only.** Read [PHASE_2C_BRIEF.md](PHASE_2C_BRIEF.md) and [ADR 0019](adr/0019-company-os-operator-surface.md), **Accepted for implementation by the owner on 2026-09-22**, with owner-approved addenda to ADR 0004 and ADR 0010 and the owner's final confirmation the same day. Nothing is implemented. For the operator surface, ADR 0019 supersedes the Phase 7 "Phase-1 UI" scope above (`apps/ops-web` on admin guessers).
...
**Classification: PHASE 2C — OWNER-APPROVED ARCHITECTURE, IMPLEMENTATION NOT STARTED.** The owner's final confirmation is recorded; implementation waits for the planning spikes. Q8 is OPEN, the production WhatsApp gate is CLOSED, no real patient data is permitted, and no production deployment is approved.
