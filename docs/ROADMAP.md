...
**Planned on `feature/phase-2c-operator-surface`, from `feature/clinical-phase-1` at `d1819f25`; planning only; pushed 2026-09-22 (architecture commit `509c3ef5`, then a docs-only lifecycle sync); PR #6 open — not merged.** Read [PHASE_2C_BRIEF.md](PHASE_2C_BRIEF.md) and [ADR 0019](adr/0019-company-os-operator-surface.md), **Accepted for implementation by the owner on 2026-09-22**, with owner-approved addenda to ADR 0004 and ADR 0010 and the owner's final confirmation the same day. Nothing is implemented. For the operator surface, ADR 0019 supersedes the Phase 7 "Phase-1 UI" scope above (`apps/ops-web` on admin guessers).
