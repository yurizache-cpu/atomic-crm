...
### Phase 2C — operator surface — OWNER-APPROVED ARCHITECTURE, IMPLEMENTATION NOT STARTED (brief 2026-09-21, owner review, final confirmation and pre-push review 2026-09-22)

**Planned on `feature/phase-2c-operator-surface`, from `feature/clinical-phase-1` at `d1819f25`; planning only; pushed 2026-09-22 (architecture commit `509c3ef5`, then a docs-only lifecycle sync); ready for PR review, no PR yet.** Read [PHASE_2C_BRIEF.md](PHASE_2C_BRIEF.md) and [ADR 0019](adr/0019-company-os-operator-surface.md), **Accepted for implementation by the owner on 2026-09-22**, with owner-approved addenda to ADR 0004 and ADR 0010 and the owner's final confirmation the same day. Nothing is implemented. For the operator surface, ADR 0019 supersedes the Phase 7 "Phase-1 UI" scope above (`apps/ops-web` on admin guessers).
...
- **Approved shape (owner decisions OD-1 to OD-15, the final confirmation and the pre-push review, [DECISIONS.md](DECISIONS.md)):**
  ...
  - **Screens (OD-9):** Overview, Activity, Tasks, Agents, Agent Runs, Reviews, Execution Stops, Costs; Communications optional as a narrow status view of counts.
...
- **"Jev"** is recorded only as the owner's name for the future provider behind the DecisionPort. Its definition, data flows and Q8 impact belong to the Phase 2D brief. Phase 2C adds no Jev SDK, API call, environment variable, schema or migration.

**Classification: PHASE 2C — OWNER-APPROVED ARCHITECTURE, IMPLEMENTATION NOT STARTED.** The owner's final confirmation is recorded; the branch is pushed, its first CI run is recorded (overall conclusion failure, baseline categories only) and it is ready for PR review; implementation waits for the planning spikes. Q8 is OPEN, the production WhatsApp gate is CLOSED, no real patient data is permitted, and no production deployment is approved.
