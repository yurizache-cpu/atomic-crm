...
| **3A** | Scheduling and follow-up | **3A.1 + 3A.2 IMPLEMENTED LOCALLY, owner visual review required** (2026-09-25; [PHASE_3A_REPORT.md](PHASE_3A_REPORT.md)): the follow-up engine and the booking foundation on the existing queue, a provider-neutral CalendarPort (Google Calendar NOT CONNECTED), and the read-only Agenda |
...
- **Next:** the owner's visual review, then the single remote integration cycle. Browser scheduling actions and Google Calendar are separate owner decisions (report §13).
