...
2. `scripts/production-scope.mjs`, with `scripts/production-scope-commands.mjs`, refuses the committed ways back that it can read:
   ...
   - function source that names a known MCP server or SQL parser, hands a raw SQL call anything but ~~a literal, a reviewed interpolation or a query Kysely compiled~~ a literal, or reaches the shared Postgres pool from anywhere but `merge_contacts`, or imports another function's files *(narrowed 2026-09-13; a raw method reached through a computed name or reflection is beyond the guard, so SI-27 seals the owner-session files)*;
