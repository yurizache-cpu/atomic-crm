...
- ~~**`ops` still does not exist.**~~ **Superseded 2026-09-12 (Phase 1A).** The schema exists (`20260912120000_ops_execution_core.sql`) with three tables, all carrying `tenant_id`, all `enable` **and** `force row level security`, all scoped by one helper. The decision in this ADR is now implemented for the engine's own tables. What it still does not cover: `public.*` remains tenant-one's CRM instance, scoped per *user* rather than per tenant, exactly as this ADR intends.
