...
## Addendum 2026-09-22 — the browser may trip, never clear (Phase 2C, owner-approved)

The owner's review of [PHASE_2C_BRIEF.md](../PHASE_2C_BRIEF.md) on 2026-09-22 approved this addendum (owner decision OD-6, accepted with amendment), together with [ADR 0019](0019-company-os-operator-surface.md), and the owner's final confirmation the same day settled its contention behaviour. **It amends the UI half of Decision 2(c)** for Phase 2C; it does not meet 2(c) as written. **Approved for implementation; not implemented.**

- **Who and where.** From the browser, a Company OS member (an owner-provisioned `tenant_operator`, not necessarily the platform owner) reaches a one-action trip at tenant-owned scopes only. The fleet-wide (`global`) trip stays one action from the CLI, which 2(c) already requires to work without the UI.
- **Trip only.** The browser may trip a stop at `tenant`, `company`, `department` or `agent` scope, inside the member's own tenant only. `global` and `job_kind` scopes are refused, and no system stop can be created from the browser.
- **Never clear.** Clearing stays a deliberate, audited act by a person through `npm run execution-stop -- clear` (this ADR's "Un-tripping is a deliberate, audited act"; SI-31). The system spend-ceiling stop stays cleared only by a person (owner decision D, 2026-09-17).
- **Through the authoritative function, under its lock.** Every browser trip calls `ops.trip_execution_stop` and takes its platform-wide exclusive kill-switch lock, which also serialises run admission. There is no separate pre-check, because one could report a stop that a concurrent clear is removing. No stop row is written directly.
- **Deny wins, unchanged.** Nothing overrides a stop, adds a force flag or bypasses the lock; a browser trip can only subtract capability.
- **Bounded contention.** The browser's trip request must never hang: its gate carries a pinned `lock_timeout` inside PostgREST's statement timeout (whether it bounds the advisory-lock wait, and the statement timeout itself, are NOT MEASURED; the Phase 2C spike S0.4 measures them and fixes the value, and if the bound cannot be shown the phase stops for owner review). On contention or timeout the API answers one generic, retryable refusal that reveals no other tenant, no other tenant's job or execution state, and no timing beyond the bound; the transaction rolls back and creates nothing. The UI may show "Stop request could not be completed yet. Retry." and never shows a stop as created before the database has committed it. Driver-backed tests exercise lock contention.
- **Origin and audit.** The member's reason (1 to 500 characters) is required and recorded, as the CLI's `--reason` is. The stop records origin `owner` (the existing non-system origin, meaning a person, not the platform owner) and the actor `principal:<uuid>` of the member the identity gate resolved. As today, a trip writes no `ops.events` row; the stop row is the record of who, when and why.
- **One action.** One control plus one confirmation, on the Overview, Agents and Execution Stops screens, available only once the trip function exists (after the Phase 2C user-management prerequisite). The CLI stays available and independent of the UI.
- **Residual, accepted by the owner (final confirmation).** The serialised behaviour is accepted: every trip takes the platform-wide lock, so a member tripping in a loop can delay other tenants' leases and starts, and a trip's lock wait is a timing signal about their activity; the bound caps a trip's own wait and the timing it can reveal, not the delay a loop of trips imposes on other tenants; spike S0.4 measures both.
- **Final owner confirmation (2026-09-22):** the existing serialised kill-switch behaviour is accepted; no separate pre-check; the authoritative stop service; deny-wins, serialisation with run admission, audit and origin preserved; a bounded request with a generic, retryable contention failure; nothing shown as created before commit.

**Status unchanged: Proposed.** This addendum approves the browser trip path only. Accepting this record as a whole, or amending it to match what Phases 1D and 1D.1 built, is still an owner decision.
