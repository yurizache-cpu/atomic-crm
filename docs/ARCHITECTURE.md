...
> 1. The isolation claim was **false** and is retracted — `ops` is unreachable *through PostgREST*, not "from any browser". The MCP function is a second, unguarded channel ([ADR 0011](adr/0011-mcp-trust-boundary.md)). *(2026-09-13: the MCP function is removed and kept out of the committed deploy paths; ADR 0011 addendum, SI-03.)*
...
~~Nothing described here is built yet. What exists today is an Atomic CRM fork; see the baseline report.~~ *(Corrected 2026-09-13: the execution substrate (Phase 1A), the worker (Phase 1B) and the Company OS domain core (Phase 1C) are built on top of the Atomic CRM fork; see §13–15 and the baseline report.)*
...
- **UUID/ULID primary keys (D3)**, client-generatable so inserts are idempotent. *(Corrected 2026-09-13, ADR 0003 as accepted: entity keys are server-generated uuids; `job_events`, `task_jobs` and `worker_instances` use other keys; no ULID exists; idempotency uses scoped idempotency keys, never a trusted caller-supplied id.)*
- **`tenant_id uuid not null`** on every table, with RLS routed through a helper function. ⚠️ **Do not copy the `02_functions.sql:462-509` pattern literally.** Every helper there resolves through `auth.uid()`, which reads a Supabase JWT claim — and the worker, the *only* process that touches `ops.*` (§3), holds no end-user JWT. The engine needs a different tenant-context mechanism: a `SET LOCAL`-scoped GUC (`set_config('app.tenant_id', …, true)`) read by the policy, set by the worker on a **non-superuser** role, per leased job. Copy the *shape* — one SECURITY DEFINER helper, referenced by every policy — not the `auth.uid()` source. Unresolved: `BASELINE_REPORT.md` §13 Q11. Settle it before the first `ops` policy is written. *(Superseded 2026-09-13: Q11 is settled by ADR 0012 as accepted. `ops.current_tenant_id()` resolves the tenant from a live lease installed by `ops.lease_job` or `ops.resume_lease`; the worker never sets a tenant GUC, and a GUC it writes cannot choose a tenant. See §13.)*
- **`force row level security`** — the baseline has none, so today the owner role (which the MCP pool uses) bypasses every policy. *(2026-09-13: every `ops` table has used FORCE since Phase 1A. It binds only a table owner without `BYPASSRLS`; `postgres` bypasses policies either way. The MCP function is removed.)*
...
- **History tables are append-only**, enforced by `REVOKE UPDATE, DELETE`. ⚠️ **Correction (2026-09-11): the repo has no example of this, and `lead_profiles` is the inverse.** Append-only means INSERT permitted, UPDATE/DELETE revoked. `lead_profiles` has no INSERT and no DELETE policy but **does** have `lead_profile_update_scoped` (`05_policies.sql:175-180`) and an explicit `grant select, update` (`06_grants.sql:25`) — a mutable current-state row that only a trigger may create. *(2026-09-17, Phase 1D.2: the forward migration `20260917180100` also created one for every contact that predated the table. The `acquired_at` of such a row is a derived backfill — the contact's earliest recorded evidence — not a recorded acquisition time (owner decision H).)* Copying it onto `ops.audit_log` would produce an audit log any authenticated user can rewrite in place, which is precisely the failure mode principle 5 exists to prevent. The append-only shape has to be **invented here**, not inherited.
...
**Communication** — `events` (append-only, the spine), `agent_messages` (structured, typed, never free-form chat), `outbox`. *(2026-09-13, ADR 0015: `ops.events` is never updated but is not append-only against the owner, which may delete for erasure and retention; the events table is itself the outbox; `seq` and every other global monotonic identifier are internal only.)*
...
**Outbox pattern.** Domain writes and their event rows commit in the same transaction; the worker publishes from the outbox. The repo's existing `pg_net` trigger is the transport primitive and also a catalogue of what to avoid: fire-and-forget, no retry, no delivery reconciliation, and a silent no-op whenever there is no end-user `Authorization` header. *(2026-09-13: `pg_net` was removed on 2026-09-11 (SI-02), and the Company OS outbox is `ops.events` itself, written in the domain transaction (ADR 0015 §7).)*
...
- **No arbitrary SQL as an agent capability.** The existing MCP `query`/`mutate` pair is the fastest agent-to-data path and the largest blast radius, and it is structurally incompatible with a replaceable CRM. Agents get typed tools. *(2026-09-13: that pair is removed with the MCP function; SI-03.)*
