...
| Function | Why |
| --- | --- |
| `ops.receive_whatsapp_message` | Takes a provider target, a message id, a sender id (or none), a body (or none) and a time; never a tenant, company, agent or task. The target selects the ONE configured channel, and only an ACTIVE TEST channel whose company, department and agent are active is live; everything happens inside that channel's tenant. Anything else answers `unrouted`: nothing is stored, and the gateway does not acknowledge it (SI-53). While the BASELINE Q8 real-data gate is closed a production channel can only be inactive, so it is never live (SI-47). A live test channel's message is admitted through the Phase 2A admission, whose identity key makes a redelivery a lookup, or refused on the record as a content-free `communication.inbound_refused` fact. The CRM is read, never written. |
...
**The closed real-data gate** *(Phase 2B pre-push review, 2026-09-18)*. `communication_channels_q8_real_data_gate` keeps every production channel inactive, for every role including the owner's own statements; `ops.configure_whatsapp_channel` refuses a live one with OS403. No role, setting or flag opens it: only a reviewed migration after the owner decides Q8, and the static migration guard reports dropping it (`constraint-dropped`).
