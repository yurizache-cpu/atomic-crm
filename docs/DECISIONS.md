...
| [0002](adr/0002-tenancy-model.md) | Tenancy — tenant is the only isolation boundary; `tenant_id` + FORCE RLS on tenant-data `ops` tables, tenant from a live lease, privilege and composite-key isolation for the Company OS; `ops` off the Data API | Proposed ⚠️ **corrected**, reconciled 2026-09-13 — MCP channel closed by removal (SI-03); **blocked on the owner-session pool in `merge_contacts`**, which no test exercises against `ops`, and on CI for the removal | **Very high** — retrofitting tenancy touches everything |
