...
| Edge function `merge_contacts` → database | One `postgres` session per function instance, `authenticated` for each transaction through `runAsUser`, with the caller's id as a bound parameter; `authenticated` holds nothing in `ops` | Measured 2026-09-13 (SI-27): reaches nothing in `ops` and carries nothing between requests. Containment is by code, not by role: a private pool and fixed statements in two sealed files (SI-27), which no other function can import (SI-03) — see [ADR 0002](adr/0002-tenancy-model.md) |
...
**No blind retry, measured rather than assumed.** No provider idempotency exists for `POST /responses`, and the official SDK retries connection errors, timeouts, 408, 409, 429 and 5xx twice unless it is configured not to, so the adapter is plain `fetch`, with zero retries explicit at the HTTP boundary. An outcome the provider's answer does not settle is recorded `indeterminate`, not `failed` (owner review, 2026-09-16): every 5xx, a timeout, a lost connection, and an unreadable or unfinished answer. The start commits before the call; any run found still `running` by anyone but its own live attempt becomes `indeterminate`, and nothing retries it. Crash, lease-expiry and shutdown cases run against real worker processes (PHASE_1D_REPORT).
...
- **Request-time refusal is an owner decision (E, 2026-09-17).** A request made while a stop covers it is still refused and recorded, with no job, no provider call and nothing deferred. A stop before admission refuses new work; a stop after admission holds the work already admitted.
